...
// Add thing to world
SOLAPI void Sol_World_Line_Add(World *world, vec3s a, vec3s b, vec3s color,
                               vec3s bColor, float ttl);
...
SOLAPI void Sol_System_Controller_Local_Tick(World *world, double dt,
                                             double time);
...
// Draw Calls
SOLAPI void Sol_Draw_Model_Instanced(SolModelId handle, uint32_t instanceCount,
...
SOLAPI void Sol_Draw_Text(const char *str, float x, float y, float size,
                          SolColor color);
...
static SystemFuncs world_systems[WORLD_SYS_COUNT] = {
    ...
    [WORLD_SYS_LINE]             = {.init = Lines_Init,
                                    .tick = Sol_System_Line_Tick,
                                    .draw = Sol_System_Line_Draw},
