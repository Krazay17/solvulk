...
int Sol_Pipeline_BuildAllDefault(SolVkState *vkstate)
{
    ...
    // ─── 3D Mesh Pipeline ───────────────────────────────────────
    if (Sol_CreateDescriptorBuffer(vkstate, sizeof(SceneUBO), VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                   VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, &sceneUBO) != 0)
        return 1;

    if (Sol_CreateDescriptorBuffer(vkstate, sizeof(ModelSSBO) * MAX_MODEL_INSTANCES, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
...
void Sol_Draw_Rectangle(SolRect rect, SolColor color, float thickness)
{
    ...
    vkCmdPushConstants(cmd, pipeRect.pipe.layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(push), &push);
...
void Sol_Draw_Model_Instanced(SolModelId handle, uint32_t instanceCount, uint32_t firstInstance)
{
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeModel.pipe.layout, 0, 2, sets, 0, NULL);

    SolGpuModel *model = &gpuModels[handle];
    for (uint32_t m = 0; m < model->mesh_count; m++)
    {
        vkCmdPushConstants(cmd, pipeModel.pipe.layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(SolMaterial),
...
void Sol_Draw_Line(SolLine *lines, int count)
{
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeRay.pipe.layout, 0, 1, &sceneUBO.sets[frame], 0,
                            NULL);
...
void Sol_Draw_Text(const char *str, float x, float y, float size, SolColor color)
{
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeText.pipe.layout, 0, 1, &pipeText.fontDesc.set, 0,
                            NULL);
    ...
    for (const char *c = str; *c; c++)
    {
        ...
        vkCmdPushConstants(cmd, pipeText.pipe.layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(SolTextPush), &push);
        vkCmdDraw(cmd, 6, 1, 0, 0);
    }
}
