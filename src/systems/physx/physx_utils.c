...
void Spatial_Hash_Tris(World *world) {
  ...
  WorldTris    *tris     = world->tris;
  int           required = HAS_MODEL;
  ...
  for (int i = 0; i < count; i++) {
    ...
    for (int i = 0; i < model->model->tri_count; i++) {
...
void collisions_hash_static(CompBody *body, CompXform *xform,
                            SubstepData substep_data, WorldSpatial *ws) {
  int totalChecks = 0;
  for (int s = 0; s < substep_data.substeps; s++) {
    xform->pos = glms_vec3_add(xform->pos,
                               glms_vec3_scale(body->vel, substep_data.sub_dt));
...
void collisions_hash_dynamic(World *world, int id, CompBody *body,
  ...
  for (int n = 0; n < 27; n++) {
    u32 entry =
        ws->table_dynamic.head[cell.neighborHashes[n] & SPATIAL_DYNAMIC_MASK];
    while (entry != SPATIAL_NULL) {
      ...
      if (id < otherID) {
        ...
        ResolveShapePair resolver =
            shape_pair_resolvers[body->shape][other_body->shape];
...
SolRayResult Sol_RaycastD(World *world, SolRay ray) {
  ...
  Sol_World_Line_Add(world, ray.pos, result.pos, (vec3s){1, 0, 0},
                     (vec3s){1, 0, 0}, 5.0f);
  if (result.hit)
    Sol_World_Line_Add(
        world, result.pos,
        glms_vec3_add(result.pos,
                      glms_vec3_scale(ray.dir, ray.dist - result.dist)),
        (vec3s){0, 1, 0}, (vec3s){0, 1, 0}, 5.0f);
  return result;
}
...
SolRayResult Sol_Raycast(World *world, SolRay ray) {
  ...
  SolRayResult result = {.dist = dist,
                         .pos = glms_vec3_add(pos, glms_vec3_scale(dir, dist))};
  WorldSpatial *ws    = world->spatial;
  SpatialGrid  *grid  = &ws->grid_static;
  ...
  if (ix < 0 || ix >= grid->dims.x || iy < 0 || iy >= grid->dims.y || iz < 0 ||
      iz >= grid->dims.z)
    return result;
  ...
  float tDeltaX =
      (fabsf(dir.x) > 1e-8f) ? grid->cellSize / fabsf(dir.x) : INFINITY;
  float tDeltaY =
      (fabsf(dir.y) > 1e-8f) ? grid->cellSize / fabsf(dir.y) : INFINITY;
  float tDeltaZ =
      (fabsf(dir.z) > 1e-8f) ? grid->cellSize / fabsf(dir.z) : INFINITY;
  ...
  while (1) {
    ...
    // Exited grid bounds?
    if (ix < 0 || ix >= grid->dims.x || iy < 0 || iy >= grid->dims.y ||
        iz < 0 || iz >= grid->dims.z)
      break;
  }
    ...
    for (int ox = -1; ox <= 1; ox++)
      for (int oy = -1; oy <= 1; oy++)
        for (int oz = -1; oz <= 1; oz++) {
          ...
          if (cx < 0 || cx >= grid->dims.x || cy < 0 || cy >= grid->dims.y ||
              cz < 0 || cz >= grid->dims.z)
            continue;
...
void spatial_static_add_model(WorldSpatial *ws, SolModel *model,
  ...
  for (u32 t = 0; t < model->tri_count; t++) {
    ...
    dst->a = glms_vec3_add(
        glms_mat3_mulv(rot, glms_vec3_mul(src.a, xform->scale)), xform->pos);
    dst->b = glms_vec3_add(
        glms_mat3_mulv(rot, glms_vec3_mul(src.b, xform->scale)), xform->pos);
    dst->c = glms_vec3_add(
        glms_mat3_mulv(rot, glms_vec3_mul(src.c, xform->scale)), xform->pos);
    ...
    dst->normal =
        len > 0.00001f ? glms_vec3_scale(cross, 1.0f / len) : (vec3s){0, 1, 0};
...
SpatialCell spatial_cell_get(vec3s pos, float cellSize) {
  ...
  for (int ox = -1; ox <= 1; ox++)
    for (int oy = -1; oy <= 1; oy++)
      for (int oz = -1; oz <= 1; oz++)
        cell.neighborHashes[n++] =
            hash_coords(cell.ix + ox, cell.iy + oy, cell.iz + oz);
  return cell;
}
...
SolCollision collide_sphere_tri(CompBody *body, CompXform *xform,
  ...
  vec3s normal =
      dist > 0.0001f ? glms_vec3_scale(delta, 1.0f / dist) : tri->normal;
  ...
  *localPos = glms_vec3_add(*localPos, glms_vec3_scale(normal, penetration));
  ...
  if (velAlongNormal < 0) {
    body->vel = glms_vec3_sub(
        body->vel, glms_vec3_scale(normal, velAlongNormal * body->restitution));
  }
...
SolCollision collide_capsule_tri(CompBody *body, CompXform *xform,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    vec3s triPoint =
        ClosestPointOnTriangle(capsulePoint, tri->a, tri->b, tri->c);
  ...
  vec3s normal =
      dist > 0.0001f
          ? glms_vec3_scale(glms_vec3_sub(bestCapsulePoint, bestTriPoint),
                            1.0f / dist)
          : tri->normal;
  float penetration = body->radius - dist;
...
SolCollision collide_sphere_sphere(CompBody *aBody, CompXform *aXform,
  ...
  aXform->pos =
      glms_vec3_add(aXform->pos, glms_vec3_scale(correctionVector, invMassA));
  bXform->pos =
      glms_vec3_sub(bXform->pos, glms_vec3_scale(correctionVector, invMassB));
