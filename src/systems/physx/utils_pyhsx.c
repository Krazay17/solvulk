...
{
    vec3s closestP = ClosestPointOnTriangle(*localPos, a, b, c);
    ...
    if (distSq < radiusSq)
    {
        ...
        // If sphere center is literally inside/on the triangle, dist is 0.
        // We calculate the triangle's actual face normal as a fallback.
        if (dist < 0.0001f)
        {
            vec3s edge1 = glms_vec3_sub(b, a);
            vec3s edge2 = glms_vec3_sub(c, a);
        ...
        // 4. Resolve Position (Push the sphere out)
        float penetration = sphereBody->radius - dist;
        *localPos = glms_vec3_add(*localPos, glms_vec3_scale(normal, penetration));

        // 5. Resolve Velocity (Bounce)
        float velAlongNormal = glms_vec3_dot(sphereBody->vel, normal);
        if (velAlongNormal < 0)
        {
            float bounce = 1.0f + sphereBody->restitution;
            vec3s impulse = glms_vec3_scale(normal, -velAlongNormal * bounce);
            sphereBody->vel = glms_vec3_add(sphereBody->vel, impulse);
        }
    }
}
...
{
    vec3s localSpherePos = glms_vec3_sub(sphereXform->pos, meshXform->pos);
    // We loop through the index array 3 at a time (one triangle)
    for (uint32_t i = 0; i < model->totalIndices; i += 3)
    {
        // Grab the actual positions using the indices
        uint32_t ia = model->indices[i + 0];
        uint32_t ib = model->indices[i + 1];
        uint32_t ic = model->indices[i + 2];

        // Convert the raw float arrays into vector structs
        vec3s a = *(vec3s *)model->vertices[ia].position;
        vec3s b = *(vec3s *)model->vertices[ib].position;
        vec3s c = *(vec3s *)model->vertices[ic].position;

        // Run the resolver against this one triangle
        ResolveSphereTriangle(sphereBody, &localSpherePos, a, b, c);
    }
    sphereXform->pos = glms_vec3_add(localSpherePos, meshXform->pos);
}

static int WorldHash(int ix, int iy, int iz)
{
    unsigned int h = ((unsigned int)ix * 73856093) ^
                     ((unsigned int)iy * 19349663) ^
                     ((unsigned int)iz * 83492791);
    return h % WORLD_TABLE_SIZE;
}
