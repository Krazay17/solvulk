...
void Shield_State_Update(World *world, int id, float dt)
{
    ...
    if (data->elapsed < ability_base[ABILITY_STATE_SHIELD].duration)
    {
        if (data->accum > HITINTERVAL)
        {
            ...
            float        size = Sol_Math_MapRange(2.0f, 4.0f, 0.0f, ability_base[ABILITY_STATE_SHIELD].duration, data->elapsed);
            ...
            for (int i = 0; i < hits; i++)
            {
                ...
                Sol_Combat_ApplyHit(world, results[i].entId,
                                    (SolHit){
                                        ...
                                        .effectMask = ability_base[ABILITY_STATE_SHIELD].effectMask,
                                        .buffMask   = ability_base[ABILITY_STATE_SHIELD].buffMask,
                                        .damage     = ability_base[ABILITY_STATE_SHIELD].damage,
                                    });
...
void Shield_State_Enter(World *world, int id)
{
    ...
    Sol_Event_Add(world, (SolEvent){
                             ...
                             .as.fx.pos      = pos,
                             .as.fx.entA     = id,
                             .as.fx.duration = ability_base[ABILITY_STATE_SHIELD].duration,
                         });
}
...
bool Shield_State_CanExit(World *world, int id, u32 next)
{
    ...
    AbilityStateData *data    = &ability->stateData[ability->activeSlot];
    return true;
    return data->elapsed >= ability_base[ABILITY_STATE_SHIELD].duration;
}
...
bool Shield_State_CanEnter(World *world, int id, u32 last, u32 next, int slot)
{
    ...
    return slot != ability->activeSlot && !(data->lastExited + ability_base[ABILITY_STATE_SHIELD].cooldown > solState.gameTime);
}
