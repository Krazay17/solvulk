...
static int Find_NextTarget(World *world, Chain *chain)
{
    ...
    for (int i = 0; i < world->chainhit->count; i++)
    {
        ...
        if (chain->accum > chain->delay)
        {
            ...
            if (target > 0)
            {
                ...
                chain->last            = target;
                if (!Sol_Combat_GetDead(world, target))
                    chain->count--;
            }
            else
                chain->count = 0;
...
void Sol_Chainhit_Trigger(World *world, int dealer, int target, u32 kind, float damage)
{
    ...
    Chain *chain           = &world->chainhit->chains[idx];
    chain->count           = chain_config[kind].chainCount;
    chain->damage          = damage;
    chain->dealer          = dealer;
    chain->last            = target;
    chain->hitEnts[target] = true;
