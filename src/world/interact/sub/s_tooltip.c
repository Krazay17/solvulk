...
void Tooltip_Card_Draw(World *world, int id, float alpha, double dt, double time)
{
    ...
    CompTooltip  *tooltip = &world->tooltips[id];
    SolItem      *item    = &world->items[id].item;
    AbilityConfig cfg     = item->ability;
    ...
    const char *headerText  = ability_names[item->abilityState];
    ...
    float totalDamage = cfg.damage + ability_base[item->abilityState].damage;
    ...
    u8 totalBuffs = cfg.buffMask | ability_base[item->abilityState].buffMask;
    ...
    u32 totalEffects = cfg.effectMask | ability_base[item->abilityState].effectMask;
