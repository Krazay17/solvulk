...
typedef void (*DrawFunc)(World *, int, float, View2 *, vec3s);

static void DrawRect(World *world, int id, float fdt, View2 *view, vec3s pos);
static void DrawCircle(World *world, int id, float fdt, View2 *view, vec3s pos);
static void DrawText(World *world, int id, float fdt, View2 *view, vec3s pos);
    ...
    for (int layer = 0; layer < UILAYER_COUNT; layer++)
    {
        ...
        for (int i = 0; i < set->cnt; i++)
        {
            ...
            for (int j = 0; j < viewComp->count; j++)
            {
                ...
                if (!xform)
                    continue;
                draw_funcs[view->kind](world, id, fdt, view, xform->draw_pos);
            }
        }
    }
}
