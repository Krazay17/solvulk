...
#pragma omp for schedule(dynamic)
        for (l = 0; l < solb_count(ws->contacts); l++)
        {
            ...
            Resolve_Contact(world, contact->idA, contact->idB, contact);
        }
    }
    ...
    Prof_EndEz(&profile, true, world->dt);
}
...
{
    int hits = Sol_Spherecast(world, ray, results, max, radius);
    ...
    *Sol_Debug_NewSphere(world, time) = (SolSphere){
        .pos    = vecAdd(ray.start, vecSca(ray.dir, ray.dist)),
        .radius = radius,
        .color  = VEC4_RED,
    };
