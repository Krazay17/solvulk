...
int Sol_Prefab_Dude(World *world, u32 id, vec3s pos, float scale)
{
    ...
    int     item_count = 4;
    SolItem items[4]   = {
        [0] = {.ability = item_rarity_base[ABILITY_STATE_CLAW][0]},
        [1] = {.ability = item_rarity_base[ABILITY_STATE_CLAW][0]},
        [2] = {.ability = item_rarity_base[ABILITY_STATE_CLAW][0]},
        [3] = {.ability = item_rarity_base[ABILITY_STATE_CLAW][0]},
    };
    Sol_Inventory_Add(world, id, (InventoryDesc){.cnt = item_count, .items = items});
    Sol_Inventory_SetAbilitySlot(world, id, 0, 0);
    Sol_Inventory_SetAbilitySlot(world, id, 1, 1);
...
int Sol_Prefab_AbilitySlot(World *world, vec3s pos, u32 slot, char *label)
{
    ...
    CompHudSlot *hud_slot = Sol_Hud_AddSlot(world, id);
    if (hud_slot)
        hud_slot->slot = slot;
...
int Sol_Prefab_HudAbilityBar(World *world)
{
    int id = Sol_Create_Ent(world, 0);
    Sol_Xform_SetPos(world, id, (vec3s){500, 350, 0});
    Sol_Body2d_Add(world, id, BODY2DKIND_RECT, ABILITYBAR_WIDTH, 70, PHYSXMASK(0b01, 0b10));
    Sol_Interact_Add(world, id);
    return id;
}