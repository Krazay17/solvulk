...
void Ability_Scripts_Init()
{
    ...
    fireballVolley->actions[1] =
        (AbilityAction){.kind = ACTIONKIND_SPAWN_PROJECTILE, .as.spawn.speed = 25.0f, .time = 0.75f};
    fireballVolley->actions[2] =
        (AbilityAction){.kind = ACTIONKIND_SPAWN_PROJECTILE, .as.spawn.speed = 30.0f, .time = 1.7f};
}
...
static void Ability_ExecuteAction(World *world, int id, AbilityAction *a)
{
    ...
    case ACTIONKIND_SPAWN_PROJECTILE:
        ...
        if (fireball > 0)
        {
            Sol_Physx_SetVel(world, fireball, vecSca(Sol_Controller_Get(world, id)->aimdir, a->as.spawn.speed));
    ...
    case ACTIONKIND_PLAY_SOUND:
        Sol_Event_Add(world, (SolEvent){
                                 ...
                                 .as.sound.pos    = Sol_Controller_GetShootPos(world, id, 1.0f),
